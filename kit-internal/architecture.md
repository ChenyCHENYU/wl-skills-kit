...
`bin/wl-skills.js` 自动从 `copilot-instructions.md` 生成 10 种 AI 编辑器配置文件（v2.3.4 新增 Qoder）。
...
迁移指南：v1.x 升级到 v2.x 只需执行 `npx @agile-team/wl-skills-kit update`，旧路径文件会自动清理，reports/ 数据保留。
...
| 阶段 | 内容                                                        |
| ---- | ----------------------------------------------------------- |
| v2.1 | dict-sync / code-fix 转正（✅ 已完成）                      |
| v2.2 | CI/CD 集成（PR 自动跑 convention-audit，不合规阻断）        |
| v2.3 | 模板版本化（每个 TPL 文件加 version frontmatter，便于回滚） |
| v2.4 | wls_code_scan / wl-skills check / diff / export / validate  |
| v3.0 | Agent Pipeline + 多语言生态                                 |
