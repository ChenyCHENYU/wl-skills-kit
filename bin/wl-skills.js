...
const FOREIGN_GENERATOR_MARKERS = [
  "由 @agile-team/wl-skills-ui 自动生成",
  "Generated by @agile-team/wl-skills-ui",
];
...
function isForeignGeneratedFile(filePath) {
  if (!fs.existsSync(filePath)) return false;
  const content = fs.readFileSync(filePath, "utf8");
  return FOREIGN_GENERATOR_MARKERS.some((marker) => content.includes(marker));
}
...
// ─── 编辑器配置生成（规则薄壳 + 原生 Skill 发现适配器）──────────────────

function getEditorConfigs(raw) {
  try {
    return buildEditorConfigs(FILES_DIR, raw);
  } catch (error) {
    console.warn("  ⚠ 多 AI 编辑器配置生成失败：" + error.message);
    return [];
  }
}
...
function collectInstallConflicts(oldManifest, validationConfig, editorConfigs) {
  ...
  const inspect = (relPath, desiredHash, sharedSourceRelPath = "") => {
    ...
    if (sharedSourceRelPath || isSharedProjectConfig(relPath)) return;
  ...
  for (const relPath of walkDir(FILES_DIR, FILES_DIR)) {
    ...
    const targetRelPath = resolveSharedProjectConfigTarget(TARGET_DIR, relPath);
    inspect(
      targetRelPath,
      fileMd5(path.join(FILES_DIR, relPath)),
      isSharedProjectConfigSource(relPath) ? relPath : "",
    );
  }
...
function prepareStaticInstall(relPath, context) {
  ...
  const targetRelPath = resolveSharedProjectConfigTarget(TARGET_DIR, relPath);
  const dest = path.join(TARGET_DIR, targetRelPath);
  if (isForeignGeneratedFile(dest)) {
    updateInstallCounter(context.stats, "preserved");
    return;
  }
  return { src, dest, targetRelPath };
}

function installStaticFile(relPath, context) {
  const prepared = prepareStaticInstall(relPath, context);
  if (!prepared) return;
  const { src, dest, targetRelPath } = prepared;
  if (isSharedProjectConfigSource(relPath) && installSharedProjectConfig(relPath, targetRelPath, src, dest, context)) {
    return;
  }
...
function installSharedProjectConfig(sourceRelPath, targetRelPath, src, dest, context) {
  const exists = fs.existsSync(dest);
  let content;
  try {
    content = desiredSharedProjectConfig(sourceRelPath, src, dest);
  } catch (error) {
    updateInstallCounter(context.stats, "preserved");
    console.warn(`  ⚠ 保留 ${targetRelPath}：共享项目配置不是可安全合并的 JSON/JSONC（${error.message}）`);
    return true;
  }
  const hash = contentMd5(content);
  context.manifest.files[targetRelPath] = hash;
  if (exists && hash === fileMd5(dest)) {
    updateInstallCounter(context.stats, "unchanged");
    return true;
  }
  if (!dryRun) {
    backupInstallConflict(context, targetRelPath, dest);
    updateInstallCounter(context.stats, writeFile(dest, content));
    return true;
  }
  console.log(`  ${exists ? "合并" : "新增"}  ${targetRelPath}  (保留注释及其他项目配置)`);
  updateInstallCounter(context.stats, exists ? "updated" : "created");
  return true;
}
...
function installEditorConfig(entry, context) {
  ...
  if (isForeignGeneratedFile(dest)) {
    updateInstallCounter(context.stats, "preserved");
    if (dryRun) console.log(`  保留  [编辑器] ${relPath}  (由 wl-skills-ui 管理)`);
    return;
  }
  const hash = contentMd5(content);
...
function shouldKeepManagedFile(relPath) {
  ...
  if (isSharedProjectConfig(relPath)) return true;
  if (isForeignGeneratedFile(path.join(TARGET_DIR, relPath))) return true;
...
function expectedManifestFiles() {
  ...
  for (const relPath of files) {
    ...
    const targetRelPath = resolveSharedProjectConfigTarget(TARGET_DIR, relPath);
    const dest = path.join(TARGET_DIR, targetRelPath);
    if (isSharedProjectConfigSource(relPath) && fs.existsSync(dest)) {
      try {
        expected[targetRelPath] = contentMd5(desiredSharedProjectConfig(relPath, src, dest));
        continue;
      } catch {
        continue;
      }
    }
    expected[targetRelPath] = fileMd5(src);
  }
...
function getValidationStagedSet() {
  ...
  console.log("  ⚠ 无 staged 的页面代码、样式或契约文件，跳过检测");
  console.log("");
  return undefined;
}
