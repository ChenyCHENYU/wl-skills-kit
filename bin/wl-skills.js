...
const SHARED_MCP_CONFIGS = new Set([
  ".mcp.json",
  ".cursor/mcp.json",
...
function installSharedMcpConfig(relPath, src, dest, context) {
  if (!fs.existsSync(dest)) return false;
  let sourceConfig;
  let targetConfig;
  try {
    sourceConfig = JSON.parse(fs.readFileSync(src, "utf8"));
    targetConfig = JSON.parse(fs.readFileSync(dest, "utf8"));
  } catch (error) {
    updateInstallCounter(context.stats, "preserved");
    console.warn(`  ⚠ 保留 ${relPath}：共享 MCP 配置不是有效 JSON（${error.message}）`);
    return true;
  }
  const merged = {
    ...targetConfig,
  ...
  console.log(`  合并  ${relPath}  (保留其他 MCP server)`);
  updateInstallCounter(context.stats, "updated");
  return true;
}
...
function shouldKeepManagedFile(relPath) {
  ...
  if (SHARED_MCP_CONFIGS.has(relPath)) return true;
