...
function collectModuleCandidates(root, pkg, runtimeJson, viteContent) {
  ...
  if (folder && folder !== path.basename(root)) addCandidate(candidates, seen, "project-folder", folder);

  const scripts = Object.values(pkg?.scripts || {}).join("\n");
  for (const match of scripts.matchAll(/--module=([A-Za-z0-9_-]+)/g)) {
    addCandidate(candidates, seen, "package-scripts", match[1]);
  }

  addCandidate(candidates, seen, "runtime-json", runtimeJson?.OPTION?.module);

  const appMatch = viteContent.match(/moduleName\s*:\s*["']([^"']+)["']/);
  addCandidate(candidates, seen, "app-config", appMatch?.[1]);

  addViewsCandidate(root, candidates, seen);
  return rankCandidates(candidates);
}

function addCandidate(candidates, seen, source, value) {
...
function addViewsCandidate(root, candidates, seen) {
  ...
  if (viewDirs.length === 1) addCandidate(candidates, seen, "views-root", viewDirs[0]);
}
...
function detectProjectShape({ standard, legacyMap, directBackend, hasVite }) {
  if (standard) return "standard";
  if (legacyMap) return directBackend ? "legacy-direct" : "legacy-gateway";
  return hasVite ? "custom" : "unsupported";
}
...
function inspectProject(rootInput) {
  ...
  const shape = detectProjectShape({ standard, legacyMap, directBackend, hasVite: Boolean(viteContent) });
