...
function planStandardEnv(rootInput, options = {}) {
  ...
  if (inspection.standard) {
    let profile;
    if (options.profile || options.profileData || options.profileFile) {
      profile = resolveProfile(options, inspection.root);
    }
  ...
  const localApi = normalizeHttpUrl(
    options.localApi || "http://localhost:10010",
    "本地后端地址",
  );
  const localPublic = normalizeHttpUrl(
    options.localPublic || "http://localhost:8002",
    "本地 public 地址",
  );
  ...
  let changes = [];
  if (profile && moduleName && errors.length === 0) {
    const managedFiles = renderManagedFiles(inspection, profile, {
      moduleName,
      localMode,
      localRoutes,
      localApi,
    ...
    changes = buildChanges(inspection.root, managedFiles);
  }
...
function formatStandardEnvResult(result) {
  ...
  if (result.inspection?.module?.candidates?.length) {
    lines.push("## 模块识别", "");
    for (const item of result.inspection.module.candidates) {
      lines.push(`- ${item.source}: ${item.value}`);
    }
    lines.push("");
  }
  if (result.inspection?.endpoints?.length) {
    lines.push("## 配置地址线索", "");
    for (const endpoint of result.inspection.endpoints) lines.push(`- ${endpoint}`);
    lines.push("");
  }
  if (result.profile) {
    lines.push("## 目标地址", "");
    for (const [key, item] of Object.entries(result.profile.environments)) {
      lines.push(`- ${key}: ${item.webUrl}/${item.apiPrefix}`);
    }
    lines.push("");
  }
  ...
  if (result.errors?.length) {
    lines.push("## 阻断", "", ...result.errors.map((item) => `- ${item}`), "");
  }
  ...
  if (result.validation) {
    lines.push(
      "## 验证",
      "",
      `- 静态验证：${result.validation.valid ? "通过" : "失败"}`,
    );
    for (const build of result.validation.builds || []) {
