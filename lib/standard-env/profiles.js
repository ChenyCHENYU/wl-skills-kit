...
function validateProfile(input) {
  ...
  return profileIdentity(input, normalized);
}
...
function profileIdentity(input, environments) {
  const rawName = input.name == null ? "custom" : input.name;
  const name = String(rawName).trim() || "custom";
  const rawTitle = input.title == null ? name : input.title;
  return {
    name,
    title: String(rawTitle).trim() || "自定义环境",
    environments,
  };
}
