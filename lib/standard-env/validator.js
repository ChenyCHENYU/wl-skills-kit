...
function validateRequiredFiles(root, inspection, errors) {
...
function validateRootConfig(inspection, errors) {
...
function validateEnvFile(root, rel, errors) {
...
function validateEnvironments(root, errors) {
  ...
  validateEnvFile(root, ".env", errors);
  validateEnvFile(root, ".env.local", errors);
}

function validateAppConfig(root, errors) {
...
function validateContextConfig(root, errors) {
...
function validatePluginConfig(root, errors) {
...
function addModuleWarning(inspection, warnings) {
  if (!inspection.module.conflict) return;
  add(
    warnings,
    "MODULE_HISTORY",
...
function validateStandardEnv(rootInput, options = {}) {
  ...
  validateRequiredFiles(root, inspection, errors);
  validateRootConfig(inspection, errors);
  validateEnvironments(root, errors);
  validateAppConfig(root, errors);
  validateContextConfig(root, errors);
  validatePluginConfig(root, errors);

  validateScripts(inspection.packageJson, errors);
  validateProfileContent(root, options.profile, errors);
  addModuleWarning(inspection, warnings);
