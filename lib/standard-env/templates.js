...
function renderManagedFiles(inspection, profile, options) {
  ...
  const pluginIndex = inspection.viteContent
    ? fs.existsSync(path.join(inspection.root, "vite/plugins/index.ts"))
      ? fs.readFileSync(path.join(inspection.root, "vite/plugins/index.ts"), "utf8")
      : ""
    : "";
  const patchedPluginIndex = patchLegacyPluginIndex(pluginIndex);
  if (patchedPluginIndex && patchedPluginIndex !== pluginIndex) {
    files["vite/plugins/index.ts"] = patchedPluginIndex;
  }
  const mainPath = path.join(inspection.root, "src/main.ts");
  if (fs.existsSync(mainPath)) {
    const main = fs.readFileSync(mainPath, "utf8");
    const patchedMain = patchLegacyMain(main);
    if (patchedMain !== main) files["src/main.ts"] = patchedMain;
  }
  return files;
}
