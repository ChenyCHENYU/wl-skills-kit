...
/** 从 index.vue 的 BaseQuery 块内捕获列数（组件缺省不可靠，编译器显式发射） */
function extractQueryColumns(vueContent) {
  if (!vueContent) return null;
  const start = vueContent.indexOf("<BaseQuery");
  if (start < 0) return null;
  const end = vueContent.indexOf("/>", start);
  if (end < 0) return null;
  const m = /:columns="(\d+)"/.exec(vueContent.slice(start, end));
...
function buildCodegenDoc(inputs, options) {
  ...
  applyQueryColumns(doc, vueContent);
  applyPatternExtraction(doc, dataContent, vueContent);
  return { doc, warnings: collectWarnings(dataContent, false) };
}

function applyQueryColumns(doc, vueContent) {
  const queryColumns = extractQueryColumns(vueContent);
  if (queryColumns) doc.queryColumns = queryColumns;
}
