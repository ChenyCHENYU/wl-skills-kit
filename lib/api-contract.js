...
// v2.21.0：默认 Profile 延迟到首次使用时读取——require 本模块（如 CLI 启动）
// 不再伴随同步磁盘 IO；冻结语义保持不变。
let cachedDefaultProfile;
function defaultProfile() {
  if (!cachedDefaultProfile) {
    cachedDefaultProfile = Object.freeze(JSON.parse(fs.readFileSync(PROFILE_FILE, "utf8")));
  }
...
function profileDeviationWarnings(profile) {
  const warnings = [];
  const baseline = defaultProfile();
  for (const name of STANDARD_OPERATIONS) {
    const actual = profile.transport?.operations?.[name];
    const expected = baseline.transport.operations[name];
  ...
  const expectedPagination = baseline.transport.pagination;
...
function loadDeliveryProfile(projectRoot, explicitFile) {
  ...
  try {
    const profile = file === PROFILE_FILE ? defaultProfile() : JSON.parse(fs.readFileSync(file, "utf8"));
    return { profile, file, errors: profileShapeErrors(profile), warnings: profileDeviationWarnings(profile) };
  } catch (error) {
    return { profile: defaultProfile(), file, errors: [`Profile 读取失败：${error.message}`], warnings: [] };
  }
}
...
module.exports = {
  // 惰性 getter：保持 DEFAULT_PROFILE 导出兼容，首次访问才读盘
  get DEFAULT_PROFILE() {
