...
// K18 逐页调用时项目依赖在一次扫描内不会变化；按项目根缓存，
// 避免 N 页项目每轮 validate 额外产生 2N 次同步读盘。
const dependencyCache = new Map();

function validationDependency(projectRoot) {
  const cacheKey = String(projectRoot || "");
...
function resetValidationDependencyCache() {
...
module.exports = {
  ...
  importedNames,
  resetValidationDependencyCache,
