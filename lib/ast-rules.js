...
// symlink 等特殊条目：跟随目标判定，保持与旧 statSync 语义一致
function walkSpecialEntry(full, base, entryName, results) {
  let stat;
  try {
  ...
  if (stat.isDirectory()) {
    if (!CONFIG.SKIP_DIRS.includes(entryName)) walkDir(full, base, results);
  } else if (stat.isFile()) {
    results.push({
      abs: full,
...
function walkDir(dir, base, results) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      if (CONFIG.SKIP_DIRS.includes(entry.name)) continue;
      ...
      walkSpecialEntry(full, base, entry.name, results);
    }
  }
  return results;
}
...
function runAstRules(targetDir, scanRel, options = {}) {
  ...
  // 扫描目标本身不存在（如误传目录）时不落缓存，避免凭空创建目录链
  if (cacheState.cacheEnabled && fs.existsSync(targetDir)) {
    writeValidationCache(targetDir, cacheState.cache);
  }
