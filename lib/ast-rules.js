 ...
 *
 * 规则编号 K1~K20 对应 standards 02/04/05/06/07/09/10/11/12/13/14 中的语义约束：
...
function readPageStyleSources(targetDir, absDir) {
  const entryPath = path.join(absDir, "index.scss");
  return collectScssSources(targetDir, entryPath);
}
...
function getPageStyleFiles(targetDir, pageDir) {
  ...
  return readPageStyleSources(targetDir, absDir)
    .map((source) => path.relative(targetDir, source.path).replace(/\\/g, "/"));
}
...
function createPageContext(targetDir, page, shared) {
  ...
  return { ...shared, page, absDir, dataContent, styleSources: readPageStyleSources(targetDir, absDir),
