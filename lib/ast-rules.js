 ...
 *
 * 规则编号 R1~R16 对应 standards 02/04/05/06/07/09/10/11/12/13 中的语义约束：
 ...
 *   R15: 分页状态/请求必须与生效 Delivery Profile 一致；无配置时回退 1/10/200 → error（11）
 *   R16: 页面直接 structuredClone / 向用户透出 error.message → warn（05/11）
 *
 * 导出函数：
...
function validateEmbeddedContract(context, source) {
  try {
    ...
    for (const item of validation.warnings.filter((item) => item.code !== "AC105")) {
      pushIssue(context, "warn", "R9", `${item.code} ${item.location}: ${item.message}`);
    }
  } catch (error) {
...
function checkEmbeddedContract(context, apiMdContent) {
  ...
  if (contractMatches.length === 0) {
    if (context.strict) pushIssue(context, "error", "R9", "严格模式要求 api.md 至少包含一个 wl-api-contract 机器块");
    return;
  }
  ...
  for (const match of contractMatches) {
    ...
    try {
      ...
      if (identities.has(identity)) pushIssue(context, "error", "R9", `api.md 存在重复 WL 机器契约：${identity}`);
...
function checkPaginationDefaults(context) {
  ...
  if (!source.trim() || hasIgnoreMarker(markerSource, "R15")) return;
...
function checkRuntimeBoundaries(context) {
  const source = `${context.scriptContent}\n${context.dataContent}`;
  const markerSource = `${context.fullSource}\n${context.dataContent}`;
  if (!source.trim() || hasIgnoreMarker(markerSource, "R16")) return;
  const findings = runtimeBoundaryFindings(source);
  if (findings.has("structuredClone")) {
    pushIssue(context, "warn", "R16", "页面直接调用 structuredClone；Vue Proxy、函数或组件实例可能触发 DataCloneError，请使用项目验证过的 cloneDeep/toRaw 序列化边界");
  }
  if (findings.has("raw-error-message")) {
    pushIssue(context, "warn", "R16", "禁止把 error.message 原样展示给用户；应优先展示后端业务 message，提供明确中文兜底，并把技术异常写入日志");
  }
}
...
// R17: 表单字段多 + 混合必填 → 提示可开启 showRequiredToggle
const R17_FIELD_THRESHOLD = 10;

function checkR17FormRequiredToggle(context) {
  const markerSource = `${context.fullSource}\n${context.dataContent}`;
  if (hasIgnoreMarker(markerSource, "R17")) return;
  // 只检测含 c_formModal 的页面
  if (!/<c_formModal\b/.test(context.template)) return;
  // 已启用则不报
  if (/show-required-toggle/.test(context.template)) return;
  // 从 data.ts 解析 formItems 中的字段数和 required 数量
  const dataSrc = context.dataContent;
  if (!dataSrc.trim()) return;
  const requiredCount = (dataSrc.match(/required\s*:\s*true/g) || []).length;
  const totalFields = (dataSrc.match(/name\s*:\s*['"`]/g) || []).length;
  if (totalFields < R17_FIELD_THRESHOLD) return;
  if (requiredCount === 0 || requiredCount === totalFields) return; // 全必填或全非必填不需要切换
  pushIssue(
    context,
    "warn",
    "R17",
    `表单 ${totalFields} 个字段（${requiredCount} 必填 / ${totalFields - requiredCount} 非必填），建议开启 show-required-toggle 支持仅必填快速填写（standards/11）`,
  );
}
