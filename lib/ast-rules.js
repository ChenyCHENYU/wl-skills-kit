 ...
 *
 * 规则编号 R1~R18 对应 standards 02/04/05/06/07/09/10/11/12/13 中的语义约束：
 ...
 *   R17: c_formModal 实际绑定的 formItems ≥10 且混合必填时建议开启快速切换（11）
 *   R18: form-validate 依赖、框架 API 和手写规则混用检查（11）
 *
 * 导出函数：
...
function checkR17FormRequiredToggle(context) {
  ...
  const form = analyzeModalForms(context.template, context.dataContent)
  ...
  pushIssue(
    ...
    "warn",
    "R17",
    `表单 ${form.binding} 有 ${form.total} 个字段（${form.required} 必填 / ${form.total - form.required} 非必填），建议开启 show-required-toggle 支持仅必填快速填写（standards/11）`,
  );
}

function checkR18FormValidationLibrary(context) {
  const source = `${context.fullSource}\n${context.dataContent}`;
  if (hasIgnoreMarker(source, "R18")) return;
  for (const finding of formValidationFindings(context.projectRoot, source)) {
    pushIssue(context, finding.level, "R18", finding.text);
  }
}
