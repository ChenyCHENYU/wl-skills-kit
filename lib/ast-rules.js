 ...
 *   R15: 分页状态默认 1/10，显式请求不得越过 1..200 边界 → error（11）
...
function checkPaginationDefaults(context) {
  ...
  for (const pair of paginationLiteralPairs(source)) {
    const validBounds = [pair.current >= 1, pair.size >= 1, pair.size <= 200].every(Boolean);
    const validDefault = !pair.owner || [pair.current === 1, pair.size === 10].every(Boolean);
    ...
    const expectation = invalidBounds
      ? "current>=1、1<=size<=200"
      : `${pair.owner} 默认分页必须为 current=1、size=10`;
    pushIssue(
      context,
      "error",
      "R15",
