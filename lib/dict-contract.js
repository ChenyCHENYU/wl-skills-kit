...
function compareDictionary(expected, actual, source, errors) {
  ...
  const expectedValues = new Set(expected.items.map((item) => item.value));
  const extraValues = actual.items
    .filter((item) => !expectedValues.has(item.value))
    .map((item) => item.value);
  if (extraValues.length > 0) {
    errors.push(
      `${source}: 字典 ${expected.code} 存在 api.md 未声明的明细 value=${extraValues.join(",")}`,
    );
  }
  if (JSON.stringify(actual.sources) !== JSON.stringify(expected.sources)) {
    errors.push(`${source}: 字典 ${expected.code} 的 sources 与 api.md 来源集合不一致`);
  }
}
...
function validateContractAlignment(moduleRoot) {
  ...
  const expectedCodes = new Set(expected.dictionaries.map((dictionary) => dictionary.code));
  const extraCodes = actual.dictionaries
    .filter((dictionary) => !expectedCodes.has(dictionary.code))
    .map((dictionary) => dictionary.code);
  if (extraCodes.length > 0) {
    errors.push(`${dictPath}: 存在 api.md 未声明的字典 ${extraCodes.join(", ")}`);
  }
