...
function fixContent(content, ext, options) {
  ...
  // F6: c_formModal 缺 show-required-toggle（仅 .vue，需同目录 data.ts 有足够字段）
  if (ext === ".vue" && options.enableFormToggle === true) {
    out = out.replace(/<c_formModal\b([^>]*?)>/g, (full, attrs) => {
      if (/show-required-toggle/.test(attrs)) return full; // 已有则跳过
...
function runSafeFix(targetDir, scanRel, options) {
  ...
  for (const abs of files) {
    ...
    // F6：只在同目录 data.ts 字段够多时启用
    let fileOptions = {};
    if (ext === ".vue" && options.enableFormToggle !== false) {
      const dir = path.dirname(abs);
      const dataFile = path.join(dir, "data.ts");
      if (fs.existsSync(dataFile)) {
        const dataContent = fs.readFileSync(dataFile, "utf8");
        const requiredCount = (dataContent.match(/required\s*:\s*true/g) || []).length;
        const totalFields = (dataContent.match(/name\s*:\s*['"`]/g) || []).length;
        if (totalFields >= 10 && requiredCount > 0 && requiredCount < totalFields) {
          fileOptions.enableFormToggle = true;
        }
      }
    }
    const { content: fixed, changes } = fixContent(content, ext, fileOptions);
