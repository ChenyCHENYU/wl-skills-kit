...
function scanDictionaryReferences(projectRoot, searchRoot) {
  ...
  const patterns = [
    /(?:logicValue\s*:\s*|useDictOpts\s*\(\s*|dictCode\s*:\s*)["']([^"']+)["']/g,
    /<jh-(?:select|radio|checkbox)\b[^>]*?\s:?dict\s*=\s*["']([^"']+)["']/g,
  ];
...
function validateDictionaryReferences(moduleRoot, options = {}) {
  const absoluteModuleRoot = fs.realpathSync(path.resolve(moduleRoot));
  const projectRoot = realProjectRoot(options.projectRoot || absoluteModuleRoot);
  if (!isInside(projectRoot, absoluteModuleRoot)) {
    throw new Error("moduleRoot 必须位于当前项目目录内");
  }
  const dictPath = path.join(absoluteModuleRoot, "dicts.ts");
  const contract = readModuleContract(dictPath);
  const allowed = new Set(contract.dictionaries.map((dictionary) => dictionary.code));
  const refs = scanDictionaryReferences(projectRoot, absoluteModuleRoot);
  const errors = [];
  for (const ref of refs) {
    if (allowed.has(ref.code)) continue;
    errors.push(
      `${relativePath(projectRoot, dictPath)}: 页面引用未登记字典 ${ref.code}（来源：${ref.sources.join(", ")}）`,
    );
  }
  return {
    ok: errors.length === 0,
