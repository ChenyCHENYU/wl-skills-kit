 ...
 * 与 page-spec 做确定性比对，输出 S1~S7 偏差：
 ...
 *   S6: 页面字段与机器 API 契约不闭合            → error
 *   S7: 进阶查询/选择回填交互契约不闭合          → error
 *
 * 设计原则：
...
function validateFeatures(spec, errs) {
  ...
  validateLookupFlows(spec.features?.lookupFlows, spec, errs);
  validateDefinitionSource(spec.features?.definitionSource, errs);
}

function validateLookupBinding(binding, flowIndex, bindingIndex, errs) {
  const location = `features.lookupFlows[${flowIndex}].bindings[${bindingIndex}]`;
  if (!binding || typeof binding !== "object" || Array.isArray(binding)) {
    errs.push(`${location} 必须是对象`);
    return;
  }
  if (!binding.source || typeof binding.source !== "string") errs.push(`${location}.source 不能为空`);
  if (!binding.target || typeof binding.target !== "string") errs.push(`${location}.target 不能为空`);
  if (typeof binding.lockAfterSelect !== "boolean") errs.push(`${location}.lockAfterSelect 必须显式声明 boolean`);
}

function validateLookupRequiredFields(flow, location, errs) {
  for (const key of ["name", "triggerAction", "queryOperation"]) {
    if (!flow[key] || typeof flow[key] !== "string") errs.push(`${location}.${key} 不能为空`);
  }
}

function validateLookupLifecycle(flow, location, errs) {
  if (!["single", "multiple"].includes(flow.selectionMode)) {
    errs.push(`${location}.selectionMode 只允许 single/multiple`);
  }
  for (const key of ["refreshOnOpen", "cancelIsolation"]) {
    if (typeof flow[key] !== "boolean") errs.push(`${location}.${key} 必须显式声明 boolean`);
  }
}

function validateLookupBindings(flow, index, location, errs) {
  if (!Array.isArray(flow.bindings) || flow.bindings.length === 0) {
    errs.push(`${location}.bindings 必须是非空数组`);
    return;
  }
  const targets = new Set();
  flow.bindings.forEach((binding, bindingIndex) => {
    validateLookupBinding(binding, index, bindingIndex, errs);
    if (binding?.target && targets.has(binding.target)) errs.push(`${location}.bindings 目标字段重复：${binding.target}`);
    if (binding?.target) targets.add(binding.target);
  });
}

function validateLookupFlow(flow, index, names, toolbarActions, errs) {
  const location = `features.lookupFlows[${index}]`;
  if (!flow || typeof flow !== "object" || Array.isArray(flow)) {
    errs.push(`${location} 必须是对象`);
    return;
  }
  validateLookupRequiredFields(flow, location, errs);
  if (flow.name && names.has(flow.name)) errs.push(`features.lookupFlows 名称重复：${flow.name}`);
  if (flow.name) names.add(flow.name);
  validateLookupLifecycle(flow, location, errs);
  validateLookupBindings(flow, index, location, errs);
  if (flow.triggerAction && !toolbarActions.has(flow.triggerAction)) {
    errs.push(`${location}.triggerAction=${flow.triggerAction} 未匹配 toolbar[].action`);
  }
}

function validateLookupFlows(value, spec, errs) {
  if (value === undefined) return;
  if (!Array.isArray(value) || value.length === 0) {
    errs.push("features.lookupFlows 必须是非空数组");
    return;
  }
  const names = new Set();
  const toolbarActions = new Set((spec.toolbar || []).map((item) => item?.action).filter(Boolean));
  value.forEach((flow, index) => validateLookupFlow(flow, index, names, toolbarActions, errs));
}
...
function machineOperations(contract) {
  const values = Object.entries(contract?.operations || {}).map(([name, operation]) => ({ name, ...operation }));
  for (const operation of contract?.extensionOperations || []) {
    if (operation?.name) values.push(operation);
  }
  return values;
}

function appendLookupIssue(issues, dir, text) {
  issues.push({ level: "error", dir, rule: "S7", text });
}

function lookupTargetFields(contracts) {
  const result = new Set();
  for (const contract of contracts) {
    for (const field of modelFieldNames(contract, ["createRequest", "updateRequest"])) result.add(field);
  }
  return result;
}

function validateLookupFlowContract(flow, flowIndex, contracts, dir, issues) {
  const location = `features.lookupFlows[${flowIndex}]`;
  const matches = contracts.flatMap((contract) => machineOperations(contract)
    .filter((operation) => operation.name === flow.queryOperation)
    .map((operation) => ({ contract, operation })));
  if (matches.length !== 1) {
    appendLookupIssue(issues, dir,
      `${location}.queryOperation=${flow.queryOperation} 必须唯一匹配机器 API 操作，实际 ${matches.length} 个`);
    return;
  }
  const { contract, operation } = matches[0];
  const { responseModel } = operation;
  const responseFields = responseModel ? modelFieldNames(contract, [responseModel]) : new Set();
  if (!responseModel || responseFields.size === 0) {
    appendLookupIssue(issues, dir, `${location} 的查询操作必须声明含字段的 responseModel`);
    return;
  }
  const targetFields = lookupTargetFields(contracts);
  for (const [bindingIndex, binding] of (flow.bindings || []).entries()) {
    if (!responseFields.has(canonicalFieldName(binding.source))) {
      appendLookupIssue(issues, dir,
        `${location}.bindings[${bindingIndex}].source=${binding.source} 不在 ${responseModel} 中`);
    }
    if (!targetFields.has(canonicalFieldName(binding.target))) {
      appendLookupIssue(issues, dir,
        `${location}.bindings[${bindingIndex}].target=${binding.target} 不在 createRequest/updateRequest 中`);
    }
  }
}

function validateLookupFlowContracts(spec, contracts, dir, issues) {
  for (const [index, flow] of (spec.features?.lookupFlows || []).entries()) {
    validateLookupFlowContract(flow, index, contracts, dir, issues);
  }
}
...
function validateSpecContractAlignment(spec, apiContent, dir) {
  ...
  validateLookupFlowContracts(spec, contracts, dir, issues);
  return issues;
}
...
function alignPage(absDir, relDir, options = {}) {
  ...
  if (error) {
    return { issues: [{ level: "info", dir: relDir, text: error, rule: "S0" }], hasSpec: false, spec: null };
  }
  if (!spec) return { issues: [], hasSpec: false, spec: null };
  ...
  return { issues: [...contractIssues, ...compareSpecToCode(spec, dataContent, relDir)], hasSpec: true, spec };
}
