...
function validateSpecContractAlignment(spec, apiContent, dir) {
    ...
    for (const [index, subTable] of spec.subTables.entries()) {
      ...
      if (matches.length !== 1) {
        issues.push({
          ...
          dir,
          rule: "S6",
          text: `subTables[${index}].resource=${subTable.resource || "<未声明>"} 必须唯一匹配一个机器契约，实际 ${matches.length} 个`,
        });
        continue;
      }
      compareSpecResource(subTable, matches[0], dir, `subTables[${index}]`, issues);
      appendFixedContextIssues(spec.features?.fixedQueryFields, matches[0], dir, `subTables[${index}]`, issues);
    }
    return issues;
  }
  if (contracts.length !== 1) {
    issues.push({
      ...
      dir,
      rule: "S6",
      text: `单资源 page-spec 必须对应唯一机器契约，实际 ${contracts.length} 个`,
    });
    return issues;
  }
  compareSpecResource(spec, contracts[0], dir, "page-spec", issues);
  appendRequiredFormIssues(spec, contracts[0], dir, issues);
  appendFixedContextIssues(spec.features?.fixedQueryFields, contracts[0], dir, "page-spec", issues);
  return issues;
}
