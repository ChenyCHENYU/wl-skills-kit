...
function validateNamedItems(spec, key, errs) {
  const values = spec[key];
  if (!Array.isArray(values)) return;
  const seen = new Set();
  for (const [index, item] of values.entries()) {
    validateNamedItem(item, key, index, seen, errs);
  }
}
...
function validateFormSections(spec, errs) {
  for (const [index, section] of (spec.formSections || []).entries()) {
    ...
    validateNamedItems({ fields: section?.fields }, "fields", errs);
  }
}

function validateSubTables(spec, errs) {
  for (const [index, table] of (spec.subTables || []).entries()) {
    validateSubTable(table, index, errs);
  }
}

function validateSubTable(table, index, errs) {
  if (!table?.name || !table?.label || !Array.isArray(table.columns)) {
    errs.push(`subTables[${index}] 必须声明 name/label/columns[]`);
  }
  validateNamedItems({ columns: table?.columns }, "columns", errs);
  if (table?.operations !== undefined && !Array.isArray(table.operations)) {
...
function validateNestedSections(spec, errs) {
  validateFormSections(spec, errs);
  validateSubTables(spec, errs);
...
function validateSpecShape(input, options = {}) {
  ...
  for (const key of ["query", "columns", "toolbar", "operations"]) {
    validateNamedItems(spec, key, errs);
  }
  validateNestedSections(spec, errs);
