...
| `env-config` | 当前前端项目根目录、客户环境 Profile、用户迁移诉求 | `.wl-skills/reports/ENV_CONFIG_*.md`（apply 时）+ 前端 env / Vite 配置 diff | `wl-skills validate` / 项目 build 验证 |
...
| `wls_env_scan` / `wls_env_apply` | 环境迁移/客户迁移/前端 baseURL 标准化 | 扫描并标准化前端 env 文件与可识别 Vite 配置，apply 默认 dry-run |
