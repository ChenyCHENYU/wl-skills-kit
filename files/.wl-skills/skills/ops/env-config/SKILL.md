...
本 Skill 只处理前端项目环境配置：`.env.*`、`env/.env.*`、`vite.config.*`、`public/env-dev.json`、Vite/uni-app baseURL 与 API 前缀线索。后端、网关、Nginx、Docker、Java、SQL、配置中心不在本 Skill 范围内。
...
- `wls_env_scan`：只读扫描，识别项目形态、现有 env 文件、可迁移的 Vite 硬编码配置和硬编码端点。
- `wls_env_apply`：默认 dry-run；只有传 `confirmApply: true` 才允许写入前端 env / Vite 配置文件。
...
3. 输出计划，明确哪些 env 文件会创建/更新、哪些 Vite/运行时配置可安全迁移、哪些字段由工具管理、哪些业务源码硬编码 URL/API 前缀需要人工确认。
...
```json
{
  "appName": "customer-a",
  "baseUrls": {
    "sit": "https://example-sit.company.com",
    "uat": "https://example-uat.company.com",
    "pre": "https://example-pre.company.com",
    "prd": "https://example-prd.company.com"
  },
  "proxyPrefixes": {
    "sit": "sit-api",
    "uat": "uat-api",
...
默认会迁移可识别的 `vite.config.*` / `public/env-dev.json` 硬编码环境映射。特殊项目只想处理 env 文件时，传 `migrateViteConfig: false` 或 CLI `--no-migrate-vite`。
