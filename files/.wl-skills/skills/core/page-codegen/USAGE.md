...
| 现象                                | 原因                                   | 解法                                                                          |
| ----------------------------------- | -------------------------------------- | ----------------------------------------------------------------------------- |
| 生成出来的页面 console 报字典查不到 | 模块 `dicts.ts` 尚未发布或线上缺项 | 校验 D1 后运行项目级 `dict-sync` 预览并安全补齐，禁止手工改线上快照               |
...
**Q：能否覆盖已有页面？**
A：默认不覆盖。明确说“覆盖现有 src/views/xxx/”即构成本次本地写入授权；AI 展示影响范围和备份策略后执行，不重复追问。若实际范围超出指定目录则必须先确认。
