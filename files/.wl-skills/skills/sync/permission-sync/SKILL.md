...
2. 若不存在，先调用 `wls_role_upsert` 预览并保存返回的 `planHash`；用户确认后再调用：
   ```json
   {
     "items": [
...
- MCP 未收到 `confirmFullReplace: true` 时会拒绝提交，防止误把增量列表当全量列表覆盖线上授权

---

## 3. action-attach（挂动作 + 加 permission 字段）
...
3. **批量新增**：先以相同 `parentId/items` 调用 `wls_action_upsert` 预览并保存 `planHash`，确认后调用：
   ```json
   {
     "parentId": "<页面菜单 id>",
     "items": [
...
- ✅ **生产环境拒绝直接 push**：`environment=production` 或网关含明确 `prod/production` 标识时，MCP 默认拒绝写入；走完发布审批后，仅可在本地显式设置 `allowProductionWrites: true`
- ✅ **写操作默认预览**：角色、动作和角色授权都必须先读取线上并取得 `planHash`；正式执行同时携带确认参数和相同哈希，线上漂移时零写入
