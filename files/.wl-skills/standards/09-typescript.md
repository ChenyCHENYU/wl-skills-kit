...
> 对应确定性执行器 **K14**（`wl-skills validate --typecheck` / MCP `wls_validate_page {typecheck:true}`，阻断级）。
...
- **优雅降级**：项目无 `tsconfig.json` 或未安装 `vue-tsc` / `tsc` 时，K14 输出 warn 不阻断（提示安装后纳入 CI）。
...
> ESLint TS 规则维持 `warn`（风格类，不阻断提交）；**类型错误阻断由 K14 兜底**（`vue-tsc --noEmit` 非零退出码 → error），二者职责分离：ESLint 管风格，K14 管正确性。
