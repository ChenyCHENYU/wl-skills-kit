...
| 环境配置 / 前端环境标准化 / 切环境 / baseURL / /api / sit-api / uat-api / 172迁移 / 客户迁移 | env-config | `.wl-skills/skills/ops/env-config/SKILL.md` |
...
4. `page-codegen` / `menu-sync` / `dict-sync` / `permission-sync` / `code-fix` / `env-config` 触发写入前二次确认
...
| 9 | 前端环境配置标准化 | env-config → wls_env_scan → wls_env_apply dry-run → 确认后 apply |
...
> 在线文档属性表基于低代码 `schema-component/{group}/{name}/props.ts`（设计器全量属性清单，最完整）维护。
> **权威源优先级**：① props.ts（最全，`lib/*.d.ts` 的 `ExtractPropTypes` 会漏掉运行时注入属性）② `lib/*.d.ts` ③ 在线文档。最终仲裁以 props.ts 为准。
...
- `env-config`（创建/更新前端环境文件；默认必须先 dry-run）
