...
效果量化：菜单同步 token 节省约 87%，从 20 分钟 10 次手动操作 → 1 分钟 0 次手动操作。
...
| 位置 | 问题 | 建议 |
|------|------|------|
| `kit-internal/skills/_planned-skills.md` | 现只剩 permission-sync 设计草稿，文件头注明已够用 | 激活 permission-sync 时顺带清理 |
...
✅  L2 Skills        — 8 个 Skill，pre-flight，registry，模板分层
✅  L3 MCP           — menu/dict 同步，4 编辑器自动配置
