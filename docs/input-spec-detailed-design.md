...
> **受众**：业务分析师 / 产品经理 / 项目经理  
> **目的**：规范详细设计文档的编写格式，使 AI 能把显式事实映射为 page-spec，并通过机器校验暴露遗漏；不承诺未经验证的准确率，也不允许猜字段名和字典码后直接进入生产代码。
...
|---|---|---|---|---|
| 详设文档（本文推荐格式） | ✅ | ✅ 直接读取 | ✅ 直接读取 | 以 page-spec/API contract 校验结果为准 |
| Axure HTML 原型 | ✅ | ⚠️ AI 推断 | ⚠️ AI 推断 | 推断项必须进入 openQuestions |
