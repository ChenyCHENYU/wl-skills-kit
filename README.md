...
**AI Skill 模板包 v2.12.2** — 一键将 14 条规范、12 个 AI Skill、19 个 MCP Tool、编辑器 MCP 配置、文档导入 Vue 3 项目。
...
│   │   │   └── env-config/       { SKILL.md, USAGE.md }  已启用
│   │   └── domain/                       领域专属（按需创建）
...
# 前端环境配置扫描 / 标准化写入（默认 dry-run）
pnpm dlx @agile-team/wl-skills-kit env scan
pnpm dlx @agile-team/wl-skills-kit env apply
pnpm dlx @agile-team/wl-skills-kit env apply --apply
...
`env-config` 面向前端项目的 `.env.*` / `env/.env.*`、`vite.config.*` 和 `public/env-dev.json`，用于统一 4/5 套环境、迁移旧 172 地址、切换客户环境，以及梳理 `baseURL` 和 `/api` / `sit-api` / `uat-api` / `prod-api` / `prd-api` 规则。后端、网关、Nginx、Docker、Java、SQL 和配置中心不在这个能力范围内。

推荐流程：

```bash
# 1. 只读扫描当前项目环境配置和硬编码 URL
pnpm dlx @agile-team/wl-skills-kit env scan

# 2. 预览将要生成或更新的前端 env / Vite 配置
pnpm dlx @agile-team/wl-skills-kit env apply

# 3. 人工确认 dry-run 报告后正式写入
pnpm dlx @agile-team/wl-skills-kit env apply --apply

# 特殊项目只想处理 env 文件时，可关闭 Vite 配置迁移
pnpm dlx @agile-team/wl-skills-kit env apply --no-migrate-vite
```
...
wls_env_apply({ migrateViteConfig: false }) # 只预览 env 文件变更
```

客户环境不一致时，把差异沉淀成 profile 文件：

```bash
pnpm dlx @agile-team/wl-skills-kit env apply --profile-file ./env-profile.customer.json
pnpm dlx @agile-team/wl-skills-kit env apply --profile-file ./env-profile.customer.json --apply
```

Profile 可按客户填写 `baseUrls` 和 `proxyPrefixes`，例如从 172 切华新、华新切客户、客户切回 172，本质都是替换环境地址；服务前缀继续按 `sit-api`、`uat-api`、`pre-api`、`prod-api` / `prd-api` 规范沉淀。

扫描和写入报告会落到 `.wl-skills/reports/ENV_CONFIG_*.md`，正式写入前会备份原文件到 `.wl-skills/reports/env-backups/`。完整用法见 [env-config 使用指南](files/.wl-skills/skills/ops/env-config/USAGE.md)。
...
| 环境     | `wls_env_scan` / `wls_env_apply`                                                                                                         |
| 项目感知 | `wls_code_scan` / `wls_route_check` / `wls_validate_page` / `wls_doctor_ui` / `wls_git_log_extract` / `wls_audit_report_push`            |

`wls_env_scan`、`wls_env_apply`、`wls_code_scan`、`wls_route_check`、`wls_validate_page`、`wls_doctor_ui`、`wls_git_log_extract` 不依赖后端 token，可用于 Agent Pipeline 前置感知项目结构。其中 `wls_env_apply` 默认 dry-run，正式写入必须传 `confirmApply: true`。
...
| `env-config`       | ✅ 启用 | `skills/ops/env-config/`        | 前端环境配置扫描、标准化与客户迁移          |
...
- 前端环境配置 Skill：[files/.wl-skills/skills/ops/env-config/USAGE.md](files/.wl-skills/skills/ops/env-config/USAGE.md)
