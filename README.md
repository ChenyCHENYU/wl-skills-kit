...
**AI Skill 模板包 v2.12.1** — 一键将 14 条规范、12 个 AI Skill、19 个 MCP Tool、编辑器 MCP 配置、文档导入 Vue 3 项目。
...
`env-config` 面向前端项目的 `.env.*` / `env/.env.*`，用于统一 4/5 套环境、迁移旧 172 地址、切换客户环境，以及梳理 `baseURL` 和 `/api` / `sit-api` / `uat-api` / `prod-api` / `prd-api` 规则。后端、网关、Nginx、Docker、Java、SQL 和配置中心不在这个能力范围内。
