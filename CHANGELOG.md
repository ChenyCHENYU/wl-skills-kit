...
- CLI 新增 `--version` / `-v` 纯版本查询，并禁止与其他命令混用。
- Skill 注册表与编辑器原生适配器统一为单一事实源；新增注册表、规则覆盖和版本漂移门禁。
- MCP HTTP 客户端复用有界 keep-alive 连接，并遵守后端 `Retry-After`。

### Fixed

- `validate` 缓存页面 `data.ts` 内容并按定义校验脚本去重，减少重复 I/O 与进程启动。
- 修复 K12 硬编码 URL 检测因清理字符串导致的漏检；现在忽略注释示例、检测真实字符串字面量。
- 修正 K19、Skill 数量及规则范围在文档、注册表和发布描述中的漂移。
...
- 文档债务清偿：README 版本亮点历史条目标号修正（v2.16.7/2.16.8 内容此前被误挂在更高版本标签下），补齐 v2.17.0 / v2.18.0 真实亮点条目。
- `guides/architecture.md` 版本表补齐 2.16.7–2.18.0 演进记录；`当前版本` 行同步真实版本与日期（此前停留在 2026-08-12）。
...
- 新增 `features.lookupFlows` 可选交互契约，显式描述进阶查询的触发动作、机器 API 操作、打开刷新、取消隔离、单/多选及来源字段到目标字段的回填锁定。
- 新增 S7 确定性门禁：查询操作必须唯一存在，响应模型字段必须覆盖回填来源，新增/更新模型必须覆盖回填目标。

### Changed

- 未声明 `lookupFlows` 的存量页面保持原行为；不按按钮文字、页面名、业务字段名或接口方法猜测交互语义。
...
- “仅必填”能力从 `c_formModal` 扩展为统一表单能力：页面 `BaseForm`、`c_formSections`、多 Tab 子表单共享 `useFormRequiredOnly`，仅混合必填/非必填的大表单显示切换，隐藏字段同步清理校验且不丢数据。
- R17 扩展到弹窗、独立页面和分区表单，并按每个实际绑定逐项判断；F6 继续只处理可安全机械补齐的弹窗 prop，不自动改写页面结构。
- R18 增加 `@robot-admin/form-validate` 3.4.1+ 版本范围检查；非标准范围明确提示人工确认，kit 自身仍不依赖该校验库。

### Fixed

- 修正独立页面模板只有切换状态、缺少子表单过滤与校验清理实现的问题。
- 修正全必填表单仍显示无效开关，以及单个已开启弹窗掩盖同页其他大表单检查的问题。
...
- 项目 Delivery Profile 成为查询方法、载荷位置和分页口径的生效事实源；包内 `POST + current=1,size=10,maxSize=200` 仅作为无项目配置时的基线。
- page-spec 新增客户端/服务端上下文来源、列表生命周期和跨字段时间顺序声明，并与 API 契约做精确集合比较。
- R15 按生效 Profile 检查分页初值和上限；项目明确采用 GET、默认 20 或其他合理口径时仅报告覆盖，不再误判为失败。

### Fixed

- 查询字段不再机械继承数据库存储长度或数值约束；只有显式 `queryConstraints` 才进入查询控件校验。
- 标准列表模板关闭普通输入失焦自动查询，补齐首次加载、搜索/重置回第一页、保存刷新和删除末页回退。
- 约束来源不足时保持待补契约，不按字段名、客户、模块或业务域猜测规则。
- Windows 全量测试改用单 thread 顺序执行，避免大量同步 CLI 黑盒子进程触发 Vitest worker RPC 假失败。
...
- 修正标准列表页模板中“新增”按钮误带 `plain: true` 的问题，创建类主按钮统一输出为客户主题 `primary` 填充样式。
- page-spec S3 同时校验工具栏按钮的颜色与 `plain` 填充形态；新增/新建/添加/创建类主按钮若不是 `primary` 填充样式，按 error 阻断。
- 列表、主从和树形列表模板为普通文本列补齐 `showOverflowTooltip: true`，列宽不足时显示省略号并在悬停时展示完整内容。
- page-codegen 规范明确排除 selection/index/操作列、自定义 Tag 渲染列以及换行列，避免长文本规则机械影响业务交互。

## [2.13.8] - 2026-07-25

### 字典同步兼容性

- `wls_dict_query` 联合查询字典树、空业务模块与全局业务模块，支持按模块编码过滤、系统模块冲突诊断，并返回脱敏目标网关与 `sysAppNo`。
- 模块、字典和明细父 ID 增加强校验，禁止 `moduleId/dictId` 为空时继续写入。
- 新增 `DICT_MODULE_SYSTEM_CONFLICT`、`DICT_MODULE_HIDDEN_CONFLICT` 等结构化失败码；后端报告已存在但查询不可见时，不再让模型盲重试或猜测历史 ID。
- 业务模块与系统模块列表支持安全分页读取，避免超过单页上限后误判不存在。
- MCP 后端客户端支持可配置业务页面上下文头、中文头值自动编码，并防止覆盖鉴权、应用和追踪头。
- 支持通过 `WL_SKILLS_TOKEN` 临时注入 token，避免自动化发布或同步场景把凭据写入本地配置。

### 文档与验证

- 补充网关显式端口/环境前缀、`dict.headers`、隐藏历史记录冲突和弱模型处理剧本。
- 新增空模块复用、缺失 ID、系统模块占用、不可见历史冲突与自定义请求头回归测试。
- README 的 MCP Tool 数量修正为实际的 23 个。

## [2.13.7] - 2026-07-23

### 变更

- 标准列表页模板固化“查询区 → 工具栏 → 列表标题 → 表格 → 分页器”顺序；按钮组必须在列表标题上方独占一行并左对齐。

## [2.13.3] - 2026-07-21

### 新增

- 标准业务组件目录扩展为 7 个有效 `local/global` 组件，支持 `component ensure --all` 一次预览并落盘全部组件。
- 新增 C4 项目实现提醒：页面生成前读取项目组件的真实 Props、Events 与 Expose。

### 变更

- 组件治理改为项目真实实现优先：已有同名有效组件或落盘后继续打磨均直接复用；已有目录缺入口时仅补齐缺失文件，任何项目文件都不覆盖。
- 组件目录标记为过渡期快照；平台能力优先，项目成熟实现经评审后人工反馈，规模稳定后可提取独立业务组件库。
- `c_formSections`、`c_spliterTitle`、`C_Tree` 等组件补齐真实运行依赖声明，全量落盘继续受 `planHash` 与跨进程锁保护。

### 移除

- `C_RightToolbar` 因引用缺失的 `old-version`、无有效项目使用且与平台工具栏能力重叠，退出目录与发布包；统一使用 `BaseToolbar` 或项目平台等价组件。
- `C_SvgIcon` 因会引入额外 `@jhlc/common-resource` 依赖退出分发；保留各项目已有 SVG 能力，有明确复用需求时再独立封装。
